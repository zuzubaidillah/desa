                        ...
                        <form name="edit" method="post" role="form" action="?tampil=galeri_editproses" enctype="multipart/form-data">
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="form-group">
                                    ...
                                    <input autocomplete="off" maxlength="60" autofocus name="judul" value="<?php echo $data['judul']; ?>" class="form-control" placeholder="Judul Menu">
                                </div>
                                <?php if ($_POST['p']=='slide') { ?>
                                <div class="form-group">
                                    <label>Keterangan</label>
                                    <input autocomplete="off" maxlength="70" name="keterangan" value="<?php echo $data['keterangan']; ?>" class="form-control" placeholder="keterangan">
                                </div>
                            <?php }else{ ?>
                                    <div class="form-group">
                                        <label>Kategori</label><br>
                                        <?php if ($data['jenis']=='galeri') { ?>
                                            <input required name="jenis" class="form-check-input" type="radio" value="galeri" checked> Portofolio
                                            <input required name="jenis" class="form-check-input" type="radio" value="client"> Client
                                        <?php }else{ ?>
                                            <input required name="jenis" class="form-check-input" type="radio" value="galeri"> Portofolio
                                            <input required name="jenis" class="form-check-input" type="radio" value="client" checked> Client
                                        <?php } ?>
                                    </div>
                                <?php } ?>
