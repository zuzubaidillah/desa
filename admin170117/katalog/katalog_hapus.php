<?php
    ...
    if (!empty($_POST['id'])) {
        $data=  mysqli_fetch_array($mysqli->query("SELECT * FROM posting WHERE idposting ='$_POST[id]' and kategori='katalog'"));
        unlink ("gambar/katalog/$data[gambar]");
        $mysqli->query("DELETE FROM posting WHERE idposting = '$_POST[id]' and kategori='katalog'") or die (mysqli_error());
            echo "
